...
const getModulePathRecursive = async (name, shift = 0) => {
  const nodeModulesParent = path.resolve(`${CWD}/${'../'.repeat(shift)}`);
  const packagePath = `${nodeModulesParent}/node_modules/${name}/package.json`;
  const isExists = Boolean(statSync(packagePath, { throwIfNoEntry: false }));
  if (isExists) return packagePath;
  if (nodeModulesParent === '/') throw new Error(`Package "${name}" not found`);
  return getModulePathRecursive(name, shift + 1);
};

const loadModule = (name) => {
  const lib = appRequire(name);
  const pkg = getModulePathRecursive(name);
